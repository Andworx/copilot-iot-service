...
3. Provision the dedicated OIDC deployment identity with the [bootstrap Bicep](../../infra/bootstrap/README.md), after inspecting its separate preview. This workflow does not create its own identity or grant itself roles. The repository's OIDC customization was verified as standard/non-immutable; the template uses the exact repository and `network-deploy` environment subject with Azure token-exchange audience.
