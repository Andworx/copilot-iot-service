...
Deployment is a subsequent explicit operation after review, **not performed by this PR**. Add a manual, main-only OIDC infrastructure workflow with protected environment, what-if review and scoped permissions before routine rollout. Existing app workflows remain authoritative for application releases. Deployment operators need network/DNS permissions on the network resource group and only the necessary private-endpoint connection approval rights on target services; avoid subscription-wide Owner/Contributor. IaC does not grant itself RBAC.
